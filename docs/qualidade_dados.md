...
| L8 | ~~NIF das empresas municipais desconhecidos~~ · **RESOLVIDA** | — | Os onze NIPC do perímetro saem do Quadro 1 de `S11-2023` e são o que torna filtráveis os contratos do BASE. O do próprio Município (`505948605`) foi resolvido do dataset do IMPIC e confirmado no timbre do Mapa de Pessoal 2026 |
| L9 | ~~Duas convenções de URL no site da CMG~~ · **RESOLVIDA** | — | O `fetch` nunca assumiu um padrão: recebe o URL completo de `sources.yaml` ou da descoberta. Na prática o site serve ambos os padrões e nenhum download falhou por isto |
...
| L33 | O dados.gov serve os ficheiros **sem `Content-Type` nem `Content-Disposition`** | Não é possível fazer com que uma ligação apenas descarregue: o navegador navega para o ficheiro | Sem `Access-Control-Allow-Origin` também não se pode descarregar por código. A ligação mantém `target="_blank"` para não se perder o dashboard, e mostra formato e tamanho para o clique ser previsível |
| L34 | O documento previsional **rotula mal algumas linhas** do mapa da despesa | Se o rótulo fosse aceite tal e qual, o dashboard atribuía despesa à unidade errada | Em 2024–2026 a unidade `01` aparece uma vez por ano com o nome "Direção Municipal de Intervenção no Território" em vez de "Administração Municipal", e há rubricas económicas com nome de unidade orgânica. O **código** e a soma das subunidades dizem qual é a certa. O parser publica o rótulo **maioritário do próprio documento**, contado sobre o mapa inteiro, e regista cada divergência em `_meta.avisos` de `orcamento_organico.json` |
| L35 | O orçamento chama **"Departamento Cultura, Economia e Inovação"** ao que o Despacho 9070/2024 renomeou para Departamento de Cultura e Turismo | A unidade `09` não liga ao organograma: `unidade_estrutura` fica a `null` | Não se força a correspondência. A ligação é por **igualdade de nome** — sem acentos e sem palavras de ligação — e onde não há igualdade o campo fica a `null` e o dashboard diz que o organograma não tem unidade com esse nome. Seis das sete unidades de topo ligam; esta não |
| L36 | A **reorganização de 2023 reaproveitou os códigos orgânicos** | Uma série 2022→2026 por código desenharia variações que não existiram | O código `03` é o Departamento de Obras Municipais em 2022 e a Direção Municipal de Intervenção no Território a partir de 2023. O gráfico de evolução por unidade começa em **2023**, e o aviso na figura diz porquê. É a mesma armadilha das siglas (L23), noutra fonte |
| L37 | A execução orçamental **não é repartida por unidade orgânica** | Sabe-se o que cada unidade tem orçamentado, não o que gastou | O mapa da despesa é previsional. Os Relatórios e Contas dão o grau de execução global (79–84%), não por unidade. A figura di-lo expressamente, para que ninguém leia o orçamento de uma direção como despesa realizada |
...
| 5 · Despesa por unidade orgânica | `etl/orcamento_organico.py` | V6 e três próprias, em cada um dos cinco anos: a soma das unidades de 1.º nível = **despesa total extraída de outro mapa por outro parser** (é esta que prova que a coluna lida é a orgânica e não a económica, que tem códigos do mesmo formato), as subunidades somam a unidade, e a repartição por natureza soma o total da unidade |
